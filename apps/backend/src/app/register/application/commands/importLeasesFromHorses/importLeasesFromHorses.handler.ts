...
import { ImportLeasesFromHorsesCommand } from "./importLeasesFromHorses.command";
...
@CommandHandler(ImportLeasesFromHorsesCommand)
export class ImportLeasesFromHorsesHandler implements ICommandHandler<ImportLeasesFromHorsesCommand> {
  ...
  async execute(command: ImportLeasesFromHorsesCommand): Promise<any> {

    const cursor = this.assetModel.find({subtype: 'horse', deleted: false, archived: false, 'data.lessees': {$exists: true, $ne: []}}).cursor();
